...
import { _internals as chaptersInternals } from "./chapters.js";


// The content track gets prepended with 28s of
// silence in step 3, so a chapter at startTime=0
// in the unshifted content actually plays at
// t=28s in the final episode. Used by the
// chapter-shift step below when produce emits an
// episode-level chapters.json next to the
// produced WAV.
const CONTENT_TIMELINE_OFFSET = 28;
...
async function run(argv) {
    ...
    // Remember the user-supplied paths before
    // any auto-conversion overwrites them. The
    // chapter-shift step below looks for
    // `<original-content>.chapters.json` next to
    // the WAV the user named, not the converted
    // copy under stage/.
    ...
    // If the content WAV the user supplied has a
    // sibling .chapters.json (presumably written
    // by `bhpodcast chapters`), shift each
    // startTime by the content-timeline offset so
    // the offsets match the FINAL episode timeline
    // (chapter 1 lands at t=28s in the episode,
    // not t=0). Emit the shifted shape in all
    // three formats next to the produced WAV.
    const contentChapters =
        `${originalContent}.chapters.json`;
    if (existsSync(contentChapters)) {
        const src = JSON.parse(
            readFileSync(contentChapters, "utf8"),
        );
        const shifted = (src.chapters || []).map((c) => ({
            startTime: Number(
                (c.startTime + CONTENT_TIMELINE_OFFSET)
                    .toFixed(3),
            ),
            title:     c.title,
        }));
        const {
            writeChaptersJson, writeChaptersTxt,
            writeChaptersCue,
        } = chaptersInternals;
        writeChaptersJson(
            shifted, `${p.output}.chapters.json`,
        );
        writeChaptersTxt(
            shifted, `${p.output}.chapters.txt`,
        );
        writeChaptersCue(
            shifted, `${p.output}.chapters.cue`,
            p.output,
        );
        process.stdout.write(
            `bhpodcast produce: shifted `
            + `${shifted.length} chapter(s) by `
            + `+${CONTENT_TIMELINE_OFFSET}s -> `
            + `${p.output}.chapters.{json,txt,cue}\n`,
        );
    }

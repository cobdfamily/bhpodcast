...
// service. The output is sized to fit the 8s
// description slot the `produce` subcommand
// expects (`silence8.wav` + description -> the
// `description.wav` stage track that plays from
// t=8s in the episode).
//
...
// 7.5s, leaving 0.5s of headroom against the
// 8s slot). The CLI exits non-zero in that case
// so a pipeline calling `bhpodcast describe`
// followed by `bhpodcast produce` fails fast
// rather than producing an episode where the
// description bleeds into the content.
...
const DEFAULT_MAX_DURATION = 7.5;
...
Optional:
  ...
  --max-duration,    <secs>  Reject the result if longer
                             than this. Default: 7.5s
                             (the produce subcommand's
                             description slot is 8s).
...
Notes:
  ...
  * Descriptions should be short. The default max
    of 7.5s keeps ~0.5s of headroom against the
    8s slot the produce step pads with.
`;

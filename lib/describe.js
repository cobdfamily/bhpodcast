...
// offset=, part=. v0.3.1 exposes voice + language;
// the others are still YAGNI.
//
// Description files (used with --text-file) can
// embed voice / language as leading `# key: value`
// comment lines:
//
//     # voice: en-US-Aria
//     # language: en-US
//
//     The actual description text starts here.
//
// CLI flags override file metadata when both are
// supplied. Parsing stops at the first line that
// doesn't match the strict `# key: value` shape,
// so a description that legitimately starts with
// `#42 hit single` (no colon) is not misread as
// metadata.
//
...
Optional:
  --voice,       -v  <name>  Voice name passed to the
                             TTS service.
  --language,    -l  <code>  Language code (BCP-47,
                             e.g. en-US, fr-CA) passed
                             to the TTS service.
...
Per-file metadata (--text-file only):
  Leading "# key: value" lines are parsed before
  the description text. Recognised keys: voice,
  language. CLI flags override file metadata when
  both are supplied. Example file:
...
// Recognises leading "# key: value" lines at the
// top of a description file as metadata. The first
// line that doesn't match the strict shape (or any
// blank line within the header block) ends the
// metadata and starts the body.
//
// Strict on purpose: a description that
// legitimately starts with "#42 hit single" (no
// colon) shouldn't be silently swallowed as
// metadata. Lines like "# Title" without a colon
// fall through to body too -- they'd otherwise look
// like a key with no value.
const HEADER_RE = /^#\s*([a-zA-Z][a-zA-Z0-9_-]*)\s*:\s*(.+?)\s*$/;
const RECOGNISED_KEYS = new Set(["voice", "language"]);


function parseHeaderMetadata(text) {
    const lines = text.split(/\r?\n/);
    const metadata = {};
    let cursor = 0;
    for (; cursor < lines.length; cursor++) {
        const line = lines[cursor];
        // Blank line: end of header block. The body
        ...
        if (!RECOGNISED_KEYS.has(key)) {
            process.stderr.write(
                `bhpodcast describe: warning -- `
                + `unknown header key '${key}' in `
                + `description file (recognised: `
                + `${[...RECOGNISED_KEYS].join(", ")}).\n`,
            );
            continue;
        }
        metadata[key] = value;
...
function resolveText(opts) {
    ...
    if (opts.text) {
        return { text: opts.text, fileMeta: {} };
    }
    if (opts["text-file"]) {
        ...
        const raw = readFileSync(opts["text-file"], "utf8");
        const { metadata, body } = parseHeaderMetadata(raw);
        return { text: body, fileMeta: metadata };
    }
    throw new Error(
...
async function fetchSpeech({
    text, voice, language, serviceUrl,
}) {
    // The newsline /v1/speak endpoint accepts the
    // text + voice + language via either query OR
    // JSON body. GET-with-query keeps the call
    // trivially cache-friendly + is what newsline's
    // own /docs examples use; descriptions are
    // short enough that URL-length isn't a concern.
...
async function run(argv) {
    ...
    const { text, fileMeta } = resolveText(opts);
    if (!text) {
        throw new Error(
    ...
    // CLI flags win over file metadata when both
    // are present. Keeps the "type-the-flag-once
    // to override" instinct working when a user
    // has a default voice in their blurb file but
    // wants to try a different one for one
    // episode.
    const voice    = opts.voice    || fileMeta.voice;
    const language = opts.language || fileMeta.language;
